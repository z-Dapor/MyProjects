		...
		return noteList;
		 ...
		 public static List<Note> changeStringToListNote(String str){
			 if(str == null || str.equals("")){
				 ...
				 List<Note> list = (List)JSONArray.toCollection(jsonarray,Note.class);
				 for (Note s : list) {
					 Note note = new Note();
			    	 note.setRowKey(s.getRowKey());
					 note.setName(s.getName());
					 note.setCreateTime(s.getCreateTime());
					 note.setStatus(s.getStatus());
					 note.setContent(s.getContent());
			    	 noteList.add(note);
			     }
				 return noteList;
			 }
...
//		 System.out.println(note.getStatus());
		 ...
		 List<Note> tt = changeStringToListNote(s2);
		 for (Note note:tt){

			 System.out.println(note.getRowKey()+";"+note.getCreateTime()+";"+note.getName()+";"+note.getContent());
		 }
